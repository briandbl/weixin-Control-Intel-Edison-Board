...
x.setKey('HHz1uWzdXuvN15FuW4DbqOvob4aVqBs2bG5Fvd2waZWAOMv4');
var dp = {
      "version":"1.0.0",
       "datastreams" : [
      {
...
x.feed.new('248508715', {
      data_point: dp,
      callback: function(e) { console.log(e); }
}); 

//x.feed.get('248508715', 
  //    { callback:function(error,response,body) {   
    //   if (!error && response.statusCode == 200) {
      ...
      // }
//});
//from chenhang method begin 

var http = require('https');
var options = {
    host: 'api.xively.com',
    path: '/v2/feeds/248508715',
    method: 'GET',
    headers: {
        'X-ApiKey': 'HHz1uWzdXuvN15FuW4DbqOvob4aVqBs2bG5Fvd2waZWAOMv4'
    }
};
var testreply;
...
//    res.setEncoding('utf-8');
    ...
    testreply="temperature:"+"("+obj.datastreams[0]+")"+"test:"+obj.led1;    
   });
   });
req.end();
...
module.exports = exports = function(webot){
  ...
  webot.set({
    ...
    handler: function(info){
      var reply = {
        title: '微信与Intel Edison Board 交互',
        pic: 'http://www.intel.com/buy/us/en/product/emergingtechnologies/intel-edison-compute-module-iot-463633',
        url: 'https://github.com/briandbl/weixin-Control-Intel-Edison-Board',
        description: [
          '你可以试试发语音信息，发送下面的指令:',
            '打开客厅灯',
            '关闭客厅灯',
...
//intel/brian/11488921 put the variable external begin 
var testreply;
//intel/brian/11488921 put the variable external end
 function do_temperature(){
...
//    res.setEncoding('utf-8');
    ...
    res.on('end',function(){
    var obj=JSON.parse(result);
    testreply="temperature:"+"("+obj.datastreams[0].current_value+")";    
   });
   });
   req.end();
   return  testreply;
  ...
  // 回复temperature 
  webot.set('reply_news', {
    description: '发送温度或者temperature,我将回复Intel CTE lab 的温度',
    pattern: /^temperature\s*(\d*)$/,
    handler:  do_temperature	
  });
  
 function  do_control_openled(){
     var dp = {
      "version":"1.0.0",
       "datastreams" : [
      {
 ...
 function   do_control_closeled(){
  ...
  webot.set('reply_news', {
    description: '关闭Intel Galileo Board LED灯',
    pattern: /^off\s*(\d*)$/,
    handler: do_control_closeled
 ...
 //所有消息都无法匹配时的fallback
  webot.set(/.*/, function(info){
	...
	log('info.param.recognition',info.param.recognition);
    info.flag = true;
	if(info.param.recognition=="打开客厅灯！"){
	    do_control_openled();
		return  "已经打开灯";
	}else if(info.param.recognition=="关闭客厅灯！"){
	      do_control_closeled();
		 return "已经关闭灯";
	}else if(info.param.recognition=="打开风扇！"){
	      do_control_openfan();
	       return "已经打开风扇";
	}else if(info.param.recognition=="关闭风扇！"){
	      do_control_closefan();
		  return "已经关闭风扇";
	}else if(info.param.recognition=="打开主卧灯！"){
	      do_control_openled();
		  return  "已经打开灯";
	}else if(info.param.recognition=="关闭主卧灯！"){
	      do_control_closeled();
		  return "已经关闭灯";
	}else if(info.param.recognition=="打开风扇关闭客厅灯！"){
	         do_control_openfan(); 
			 do_control_closeled();
			 return "已经执行";
	}else if(info.param.recognition=="关闭风扇打开客厅灯！"){
	        do_control_closefan();
			do_control_openled();
			return "已经执行";
	}else if(info.param.recognition=="打开风扇打开客厅灯！"){
		   do_control_openled();
		   do_control_openfan();
		   return "已经执行";
	}else if(info.param.recognition=="关闭风扇关闭客厅灯！"){
	        do_control_closeled();
			do_control_closeled();
			return "已经执行";
	}else if(info.param.recognition=="外出模式！"){
	        do_control_closeled();
			do_control_closefan();
			return "已经执行";
	}else if(info.param.recognition=="在家模式！")｛
	       do_control_openled();
		   do_control_openfan();
		   return "已经执行";
	｝
